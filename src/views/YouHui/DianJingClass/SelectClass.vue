...
<template>
  <div id="YH80To1000_class" class="select-class">
    <div class="select-title">
      ...
      <!-- <p @click="showRegionCampus" class="boright" style="width: 85% !important">
        <span class="level">{{selectedRegion || '上课时间'}}</span>
        <img v-if="selection.key === 'region'" :src="PNGs.arrowDown" alt="">
        <img v-else :src="PNGs.arrowUp" alt="">
      </p> -->
    </div>
...
export default {
  ...
  computed: {
    regionList () {
      ...
      this.classData.map(x => {
        if (list.indexOf(x.classTime) < 0) list.push(x.classTime)
      })
    ...
    campusList () {
      ...
      this.classData.map(x => {
        if (list.indexOf(x.filterCampus) < 0) list.push(x.filterCampus)
      })
    ...
    filterClasses () {
      ...
      if (this.selectedRegion) {
        list = this.classData.filter(x => x.classTime === this.selectedRegion)
      }

      if (this.selectedCampus) {
        list = this.classData.filter(x => x.filterCampus === this.selectedCampus)
      }
  ...
  data () {
    return {
      ...
      classData: mockData.dianJingClass
    }
  },
